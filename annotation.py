...
def parse_node(lines: List[str], cur_lvl_start="") -> Tuple[Node, int]:
    ...
    if len(splitted) == 2:
        ...
        raise ValueError(
            f"failed to parse line0 into node_name and costs: {lines[0]}")
    ...
    if len(splitted) >= 2:
        ...
        if list(actual_parser.keys())[0] in splitted[1]:
            # explain analyze
            actual_cost = splitted[1][:-1]
        else:
            # explain but with some other stuffs like (not estimated)
            actual_cost = ""
    else:
        # explain
        estimated_cost, actual_cost = splitted[0][1:-1], ""
    estimated = {}
    ...
    while line_idx < len(lines):
        ...
        if line_content.startswith("->"):
            child, next_index = parse_node(
                lines[line_idx:], cur_lvl_start=cur_lvl_start+BIG_STEP)
            children.append(child)
            line_idx += next_index
        else:
            splitted = line_content.split(": ")
            # "Worker 1:  Sort Method: quicksort  Memory: 27kB", ignore for now
            if len(splitted) > 2:
                line_idx += 1
